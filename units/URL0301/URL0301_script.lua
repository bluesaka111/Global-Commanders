...
function printecnhancement(unit)
    if getEnh.GetEnhancements(unit:GetEntityId()) != nil then
        for k,v in getEnh.GetEnhancements(unit:GetEntityId()) do
            LOG('*SACU: ',k..' '..v)
        end
        LOG('*****************')
    end
...
function CopyEnhancement(srcUnit, destUnit)
    local entId = srcUnit:GetEntityId()
...
URL0301 = Class(CWalkingLandUnit) {
    ...
    AirMode = function(self)
        if not self:IsDead() then
            ...
            local ura0301 = CreateUnit('ura0301', self:GetArmy(), location[1], location[2], location[3], myOrientation[1], myOrientation[2], myOrientation[3], myOrientation[4], 'Land')
            ura0301:AddKills(numkills)
            CopyEnhancement(self, ura0301)
            ura0301:SetHealth(self,health)
            ura0301:SetScriptBit('RULEUTC_WeaponToggle', true)
            ura0301:SetScriptBit('RULEUTC_CloakToggle', self:GetScriptBit('RULEUTC_CloakToggle'))
            ura0301 = nil
